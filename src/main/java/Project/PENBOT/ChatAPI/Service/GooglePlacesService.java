...
public class GooglePlacesService {
    ...
    public List<PlaceInfoDTO> searchNearby(String keyword, String type) {
        ...
        ResponseEntity<PlaceInfoDTO> response = restTemplate.getForEntity(url, PlaceInfoDTO.class);

        List<PlaceInfoDTO> result = new ArrayList<>();
        if (response.getStatusCode().is2xxSuccessful()) {
            try {
                JsonNode root = new ObjectMapper().readTree(String.valueOf(response.getBody()));
